...
namespace blqw.Reflection
{
    ...
    public static class CSCommentReader
    {
        ...
        public static CSComment Create(MethodInfo method)
        {
            if (method == null)
            {
        ...
        private static string GetDescription(MemberInfo member)
        {
            ...
            {
                var disAttr = (DisplayNameAttribute)Attribute.GetCustomAttribute(member, typeof(DisplayNameAttribute));
                if (disAttr != null)
                {
                    return disAttr.DisplayName ?? member.Name;
                }
            }
            return member.Name;
        }
    ...
    public static class CSCommentXMLCache
    {
        ...
        internal static string GetXmlPath(string dllpath)
        {
            ...
            {
                xmlfile = Path.Combine(System.AppDomain.CurrentDomain.BaseDirectory, "bin", Path.GetFileNameWithoutExtension(dllpath) + ".xml");
                if (File.Exists(xmlfile) == false)
                {
                    return null;
                }
