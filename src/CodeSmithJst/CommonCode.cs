...
{
    /// <summary>
    /// 公共代码帮助类  由于CodeSmith 不支持多层继承，因此只有所有代码都写在这一个类中，继承CodeTemplate
    /// </summary>
    public class CommonCode : CodeTemplate
    {
        ...
        public void RenderToFileByTables(bool isRender, string templatePath, string directory, TableSchemaCollection tables, Func<TableSchema, string> className = null)
        {
            if (isRender)
            {
                ...
                {
                    //CopyPropertiesTo(template);
                    template.SetProperty("Table", tab);
        ...
        public static bool TryToBool(object str, bool defaultResult = false)
        {
            ...
            {
                bool tmp;
                if (bool.TryParse(str.ToString(), out tmp))
                {
                    return tmp;
                }
            }
        ...
        public static string GetNoSplitName(string name)
        {
            if (string.IsNullOrWhiteSpace(name))
            {
                return name;
            }
            var match = new Regex(@"_\d*$").Match(name);
            return match.Length > 0 ? name.Remove(name.Length - match.Length) : name;
        }
        #endregion Helper方法
